  ...
  try {
    ...
    if (!file || file.size === 0) {
      return Response.json(
        { success: false, error: 'Không tìm thấy file. Vui lòng chọn một file 3D để tải lên.' },
        { status: 400 }
      )
    }
    ...
    if (file.size > MAX_FILE_SIZE) {
      ...
      return Response.json(
        {
          success: false,
          error: `File quá lớn (${sizeMB}MB). Dung lượng tối đa cho phép là 50MB.`,
        },
        { status: 400 }
      )
    }
    ...
    const uploadResult = await new Promise<any>((resolve, reject) => {
      ...
      console.error('[upload-3d] Cloudinary upload error:', uploadError)
    ...
    console.error('[upload-3d] Unexpected error:', error)
    return Response.json(
