...
export default function EditPostForm({ post }: EditPostFormProps) {
  ...
  const insertImageIntoContent = (url: string) => {
    ...
    // Chèn tag ảnh HTML với class mặc định tại vị trí con trỏ chuột
    ...
    // Trả con trỏ chuột về lại vùng soạn thảo sau hình ảnh vừa được chèn
    setTimeout(() => {
      textarea.focus()
  ...
  const confirmSave = async () => {
    ...
    const res = await updatePost(post.id, { title, content, published, imageUrl })
    setSubmitting(false)
      ...
      <Dialog open={showSaveConfirm} onOpenChange={setShowSaveConfirm}>
        <DialogContent>
          <DialogHeader>
            ...
            <DialogDescription>
              Bạn có chắc chắn muốn lưu các thay đổi cho bài viết này không?
            </DialogDescription>
          </DialogHeader>
          <DialogFooter>
            <button
